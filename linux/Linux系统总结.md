...
![图片](Linux系统总结.assets/640-1647870463875.jpg)
...
![图片](Linux系统总结.assets/640-1647870632286.jpg)
...
![图片](Linux系统总结.assets/640-1647870699348.jpg)
...
![图片](Linux系统总结.assets/640-1647870773952.jpg)
...
[外链图片转存中...(img-fjmDiQ1c-1647699612873)]
...
[外链图片转存中...(img-aiZLckDZ-1647699612874)]
...
[外链图片转存中...(img-1o5DAOKG-1647699612876)]
...
[外链图片转存中...(img-xdRA9uvO-1647699612878)]
...
在 `Linux` 中一个命令的去向可以有3个地方：终端、文件、作为另外一个命令的入参。[外链图片转存中...(img-ijijrsEo-1647699612880)]命令一般都是通过键盘输入，然后输出到终端、文件等地方，它的标准用语是 `stdin` 、 `stdout` 以及 `stderr` 。

- 标准输入 `stdin` ，终端接收键盘输入的命令，会产生两种输出；
- 标准输出 `stdout` ，终端输出的信息（不包含错误信息）；
- 标准错误输出 `stderr` ，终端输出的错误信息。

[外链图片转存中...(img-2qmS0BET-1647699612882)]
...
[外链图片转存中...(img-5py7DsmX-1647699612884)]
...
[外链图片转存中...(img-haQ8Ek9V-1647699612885)]
...
[外链图片转存中...(img-ktu9FRsc-1647699612887)]
...
[外链图片转存中...(img-yEi0ag0r-1647699612889)]
...
用一张图表示三种模式如何切换：[外链图片转存中...(img-HMvIGVmR-1647699612891)]
